...
import { getConfigPath, resolveTools, loadWorkspaceContext, loadSkillsContext, InferenceEngine } from 'hyperclaw/core';
import type { InferenceMessage } from 'hyperclaw/core';
...
async function printSkills(): Promise<void> {
  console.log();
  try {
    const { loadSkills } = await import('hyperclaw/core');
...
export async function runChat(opts: {
  ...
  const prompt = () => {
    rl.question(chalk.bold.green('  You › '), async (input) => {
      ...
      try {
        const { AutoMemory } = await import('hyperclaw/core');
